...
def work(getGradeUrl):
    ...
    for school in schools[0:1]:
        ...
        for grade in grades[0:1]:
            ...
            for course in getCourses(gradeCode)[-3:-2]:
                        ...
                        if(bool(bookUuid)):
                            ...
                            for chapter in chapters:
                                if(chapter['parent'] == chapter['teaching_material']):
                                    topic = bookDst + chapter['title'] + '/'
                                    lastTopic = chapter['title']
                                    #topics.update(chapter['identifier']=topic)
                                    topics['%s' % chapter['identifier']] = topic
#                                    ensureDir(bookDst + '/' + topic + '/')
                                    print '\n--------' + topic
                                else:
                                    lastPos = bookDst + lastTopic + '/' + chapter['title']
                                    ...
                                    topics['%s' % chapterIdentifier] = sectionDst
#                                    ensureDir(sectionDst)
                                    try:
                                        print lastPos
                                        print sectionDst
                                        shutil.move(lastPos, sectionDst)
                                    except:
                                        print 'have moved , download a new file'
                                    else:
